...
class Nexcessnet_Turpentine_Model_Observer_Esi extends Varien_Event_Observer {
	...
	if( $esiHelper->getEsiBlockLogEnabled() ) {
            $debugHelper->logInfo(
                'Checking ESI block candidate: %s' . '; block_class:' . get_class($blockObject),
                $blockObject->getNameInLayout() );
 $debugHelper->logInfo(
                'request params:  %s' ,
                'Should use esi: ' . strval($esiHelper->shouldResponseUseEsi()) . "\n"
		. 'correct instance: ' . strval($blockObject instanceof Mage_Core_Block_Template) . "\n"
		. 'have esi opts: ' . is_array($blockObject->getEsiOptions())
 );

        }
$additional = false;
if (($blockObject->getNameInLayout() == "compareSidebarLeft")){
$additional = true;
$esiOptions = array(
            $esiHelper->getEsiMethodParam()         => 'esi',
            $esiHelper->getEsiScopeParam()          => 'global',
            $esiHelper->getEsiCacheTypeParam()      => 'private',
            'dummy_blocks'      => array(),
            'registry_keys'     => array(),
        );
