...
class Nexcessnet_Turpentine_EsiController extends Mage_Core_Controller_Front_Action {
    ...
    public function getBlockAction() {
        $resp = $this->getResponse();
        $cacheFlag = false;
        //Mage::Log("getBlockAction::shouldResponseUseEsi?");
        if( Mage::helper( 'turpentine/esi' )->shouldResponseUseEsi() ) {
            //Mage::Log("getBlockAction::should!!!");
            $req = $this->getRequest();
            ...
            if( $esiDataHmac !== ( $hmac = $dataHelper->getHmac( $esiDataParamValue ) ) ) {
                ...
                //Mage::log("getBlockAction::getting esi data");
    ...
    protected function _getEsiBlock( $esiData ) {
        ...
        //Mage::log('in here: ' . $esiData->getNameInLayout());
        if ($esiData->getNameInLayout() == "compareSidebarLeft") {
        #    print_r(array("aaaa"));exit();
        }
        foreach( $esiData->getComplexRegistry() as $key => $data ) {
            $value = Mage::getModel( $data['model'] );
            //Mage::log("Model in use: {$data['model'] }");
            if( !is_object( $value ) ) {
