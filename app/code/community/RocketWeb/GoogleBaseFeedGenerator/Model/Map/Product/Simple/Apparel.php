...
class RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Simple_Apparel extends RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Simple {
     ...
     * @return string
     */
    public function mapDirectiveApparelSizeType($params = array()) 
    {
        $map = $params['map'];
        $cell = "";
        $allowedSizeType = $this->getConfig()->getAllowedSizeType();

        $default_value = isset($map['default_value']) ? $map['default_value'] : "";
        if ($default_value != "") {
            if (count(array_diff($this->getTools()->explodeMultiselectValue(strtolower($default_value)), $allowedSizeType)) == 0) {
                $cell = $default_value;
                $cell = $this->cleanField($cell, $params);
                return $cell;
            }
        ...
        $attributes_codes = $this->getConfig()->getMultipleSelectVar('size_type_attribute_code', $this->getStoreId(), 'apparel');
        if (count($attributes_codes) == 0 ) {
            $cell = "";
            return $cell;
        }
        ...
        foreach ($attributes_codes as $attr_code) {
            ...
            $cell = $this->cleanField($this->getAttributeValue($this->getProduct(), $attribute), $params);
            if ($cell != "") {
                if (count(array_diff($this->getTools()->explodeMultiselectValue(strtolower($cell)), $allowedSizeType)) == 0) {
                    break;
                } else {
                    $cell = "";
                }
     ...
     * @return string
     */
    public function mapDirectiveApparelSizeSystem($params = array())
    {
        $map = $params['map'];
        $cell = "";
        $allowedSizeSystem = $this->getConfig()->getAllowedSizeSystem();

        $default_value = isset($map['default_value']) ? $map['default_value'] : "";
        if ($default_value != "") {
            if (count(array_diff($this->getTools()->explodeMultiselectValue(strtolower($default_value)), $allowedSizeSystem)) == 0) {
                $cell = $default_value;
                $cell = $this->cleanField($cell, $params);
                return $cell;
            }
        ...
        $attributes_codes = $this->getConfig()->getMultipleSelectVar('size_system_attribute_code', $this->getStoreId(), 'apparel');
        if (count($attributes_codes) == 0 ) {
            $cell = "";
            return $cell;
        }
        ...
        foreach ($attributes_codes as $attr_code) {
            ...
            $cell = $this->cleanField($this->getAttributeValue($this->getProduct(), $attribute), $params);
            if ($cell != "") {
                if (count(array_diff($this->getTools()->explodeMultiselectValue(strtolower($cell)), $allowedSizeSystem)) == 0) {
                    break;
                } else {
                    $cell = "";
                }
