    ...
    */
    ...
    protected function mapDirectivePrice($params = array()) {
        ...
        if ($includingTax) {
            ...
            $this->_cache_price_including_tax = $helper->getPrice($product, $convertedPrice, true, false, false, null, $store, $priceIncludesTax);
            $cell = $this->_cache_price_including_tax;
        } else {
            $this->_cache_price_excluding_tax = $helper->getPrice($product, $this->getPrice($product), $includingTax, false, false, null, $this->getStoreId(), $priceIncludesTax);
    ...
    protected function mapDirectiveSalePrice($params = array()) {
        ...
        if ($this->hasParentMap() && $this->getParentMap()->getProduct()->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE) {
    ...
    protected function mapAttributeWeight($params = array()) {
        ...
        $weight .= $weight ? ' '.$this->getConfigVar('weight_unit_measure', 'columns') : '';
    ...
    protected function mapDirectiveProductTypeMagentoCategory($params = array()) {
        ...
        $cats = $product->getCategoryCollection()->count();

        if ($this->hasParentMap() && !$cats) {
            return $this->getParentMap()->mapColumn($params['map']['column']);
        }

        return $this->cleanField(Mage::getSingleton('googlebasefeedgenerator/config')->getProductCategoryTree($product, $params), $params);
    }
    ...
    /**
     * Free Map memory - important FIX: memory / _segmentfault_ errors
     * New FIX: I had to comment out this method as there was a segmentfault at first line under RocketWeb_GoogleBaseFeedGenerator_Model_Generator::addProductToFeed()
     */
...
//    }
    ...
    public function isDuplicate() {
        ...
        $process->setParentItemId($this->hasParentMap() ? $this->getParentMap()->getProduct()->getEntityId() : null);
