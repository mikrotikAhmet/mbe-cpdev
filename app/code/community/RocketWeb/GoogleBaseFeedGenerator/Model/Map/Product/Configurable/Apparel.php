...
class RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Configurable_Apparel extends RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Simple_Apparel {
    ...
    public function map() {
        ...
        foreach ($assocIds as $assocId) {
            ...
            if (!$this->getConfigVar('add_out_of_stock_configurable_assoc') && $stock != $this->getConfig()->getInStockStatus()) {
                $is_skip = true;
    ...
    public function validateVariants(&$rows) {
    	...
    	foreach ($rows as $line => $row) {
            ...
            if ($missing_columns) {
                $this->log(sprintf('Product [id: %s, mpn %s] is missing the apparel variant columns %s Please add those columns to the feed.', array_key_exists('id', $row) ? $row['id'] : '', array_key_exists('mpn', $row) ? $row['mpn'] : '', $missing_columns));
            }
