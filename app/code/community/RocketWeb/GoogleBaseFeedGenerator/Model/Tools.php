...
class RocketWeb_GoogleBaseFeedGenerator_Model_Tools extends Varien_Object {
    ...
    public function setCacheAssociatedPricesByProduct($map, $assoc) {
        ...
        if (!$has_option_price && count($configurable_attributes)) { // TODO: figure out why this is needed.
    ...
    public function lockDuplicates($row, $is_assoc_configurable, $is_assoc_grouped) {
        ...
        if ($is_assoc_configurable !== false
            ...
            foreach ($is_assoc_configurable as $assoc) {
                if (!in_array($assoc['parent_entity_id'], $parents)) {
                    array_push($parents, $assoc['parent_entity_id']);
                }
            }

            if ($generator->getConfigVar('log_skip')) {
                $prods_msg = ': ';
                foreach ($is_assoc_configurable as $row) {
                    $prods_msg .= sprintf('[SKU %s, ID %d]', $row['parent_sku'], $row['parent_entity_id']) . (end($is_assoc_configurable) ? '': ',');
                }
        ...
        if ($is_assoc_grouped !== false
            ...
            if ($generator->getConfigVar('log_skip')) {
                $prods_msg = '';
                foreach ($is_assoc_grouped as $row) {
                    $prods_msg .= sprintf('[SKU %s, ID %d]', $row['parent_sku'], $row['parent_entity_id']) . (end($is_assoc_grouped) ? '': ',');
                }
        ...
        Mage::getModel('googlebasefeedgenerator/process')->setData(array(
