...
class RocketWeb_GoogleBaseFeedGenerator_Model_Generator extends Varien_Object {
    ...
    public function run() {
        ...
        if ($completedForToday = $this->getBatchMode() ? $this->getBatch()->completedForToday() : false) {
            Mage::throwException(sprintf('Script %s can\'t run. The feed was completly generated once today %s', $this->getScheduleId(), date('Y-m-d')));
        }
    ...
    public function processProductCallback($args) {
        ...
        if ($is_assoc_configurable || $is_assoc_grouped) {
            $this->getTools()->lockDuplicates($row, $is_assoc_configurable, $is_assoc_grouped);
            return;
        }
