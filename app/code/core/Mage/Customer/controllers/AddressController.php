...
class Mage_Customer_AddressController extends Mage_Core_Controller_Front_Action
{
    ...
    public function formPostAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $newAddress = true;
            $oldAddressInfromation = Mage::getModel('customer/address');
            if ($addressId) {
                $existsAddress = $customer->getAddressById($addressId);
                if ($existsAddress->getId() && $existsAddress->getCustomerId() == $customer->getId()) {
                    $address->setId($existsAddress->getId());
                    $oldAddressInfromation->load($existsAddress->getId());
                    $newAddress = false;
                }
            }
            ...
            try {
                ...
                if (count($errors) === 0) {
                    $address->save();
                    if ($newAddress === true) {
                        Mage::dispatchEvent('customer_address_added',
                            array('address_controller' => $this, 'customer' => $customer, 'address'=> $address)
                        );
                    } else {
                        Mage::dispatchEvent('customer_address_saved',
                            array('address_controller' => $this, 'customer' => $customer, 'address'=> $address, 'oldaddress'=>$oldAddressInfromation)
                        );
                    }
    ...
    public function deleteAction()
    {
        ...
        if ($addressId) {
            ...
            try {
                $customer = $this->_getSession()->getCustomer();
                Mage::dispatchEvent('customer_address_removed',
                            array('address_controller' => $this, 'customer'=>$customer, 'address'=> $address)
                );
                $address->delete();
